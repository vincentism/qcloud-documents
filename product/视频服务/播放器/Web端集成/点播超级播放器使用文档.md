...
### 弹幕
当您想要使用弹幕相关功能时，可以通过将 plugins 中的 Danmu 参数设置为 true 来开启弹幕功能，开启之后，播放器的控制栏将会显示弹幕图标，用于控制弹幕是否在视频画面上显示。同时您可以通过播放器初始化返回的对象访问到弹幕管理器 CommentManager，通过弹幕管理器可以实现诸如启动、加载、暂停弹幕以及其他控制行为，示例代码如下：
...
// 加载弹幕列表
player.CommentManager.load(danmakuTimeline);
player.CommentManager.start();
// 显示/隐藏弹幕的回调方法
player.on('danmutoggle', data => {
  // do something
});

```
>!弹幕功能基于 CommentCoreLibrary 实现，详情请参考 [开发文档](https://github.com/jabbany/CommentCoreLibrary)
